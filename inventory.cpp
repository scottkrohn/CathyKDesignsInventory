...
{
	double value = 0;
	for(std::vector<Stock>::const_iterator i = items.begin();
		i != items.end(); ++i)
	{
		value += i->itm.total();
	}
...
void Inventory::outputToFile() const
{
	...
	for(std::vector<Stock>::const_iterator i = items.begin(); i != items.end(); ++i)
	{
		// Is this the first time writing to the file?
		if(i != items.begin())
		{
			// If not, add a new line before writing
			outfile << '\n';
		}
		outfile << *i;
	}
...
void Inventory::loadInventory()
{
	...
	while(!infile.eof())
	{
		infile >> name;
		infile >> totYds;
		infile.get(sentinal);		// Ignore first ','
		std::vector<std::pair<double,double>> buys;
		while(sentinal != ';')
		{
			double y;				// Yards
			double cpy;				// Cost Per Yard
			infile.ignore(1);		// Ignore '('
			infile >> y;
			infile.ignore(1);		// Ignore ','
			infile >> cpy;
			infile.ignore(1);		// Ignore ')'
			infile.get(sentinal);	// Get char to check for ';'
			buys.push_back(std::pair<double,double>(y,cpy));
		}
		items.push_back(Stock(name, Item(buys)));
		infile.ignore(1);	// Ignore newline char
	}
}
